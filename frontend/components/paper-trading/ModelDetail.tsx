...
import { fmt, CatMeta, Model, defaultBacktestWindow, holdingWeights, metricStateLabel, metricStateTip } from "./ptdata";

interface Live { equity: Pt[]; benchmark?: Pt[]; holdings: Model["holdings"]; cagr: number; maxDD: number; winRate: number; trades: number; sharpe: number | null; window: { start: string; end: string } }
...
export default function ModelDetail({ model, cat, onClose, onEdit, onBacktest, onDelete, onBacktested }: {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setLive({
          equity, benchmark, holdings: holdingWeights(run.holdings || []),
          cagr: (final / start - 1) * 100, maxDD: drawdownOf(equity).maxDD,
  ...
  const stats = live
    ...
    : model.stats;
  const holdings = live?.holdings ?? model.holdings;
  const up = !isLive || totalRet >= 0;
      ...
      <DetailHeader onClose={onClose}>
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            <span title={live ? "Generated by the backtest that just completed." : metricStateTip(model)} style={{ fontSize: 10.5, fontWeight: 600, padding: "2px 7px", borderRadius: 999, color: isLive ? "var(--pos)" : "var(--text-3)", background: isLive ? "var(--pos-soft)" : "var(--surface-3)" }}>
              {isLive ? "Backtested" : metricStateLabel(model)}</span>
          </div>
      ...
      <DetailBody gap={26}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 14, marginBottom: 4, flexWrap: "wrap" }}>
            <span className="mono" style={{ fontSize: 34, fontWeight: 600, color: isLive ? (up ? "var(--pos)" : "var(--neg)") : "var(--text-3)" }}>{isLive ? fmt.pct(totalRet) : "—"}</span>
        ...
        <div className="m-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          <MetricTile subtle label="Return" value={isLive ? fmt.pct(stats.cagr) : "—"} tone={isLive ? (stats.cagr >= 0 ? "pos" : "neg") : undefined} />
          <MetricTile subtle label="Sharpe" value={stats.sharpe != null ? stats.sharpe.toFixed(2) : "—"} />
          <MetricTile subtle label="Max DD" value={isLive ? stats.maxDD.toFixed(1) + "%" : "—"} tone={isLive ? "neg" : undefined} />
        ...
        {holdings.length > 0 ? <section>
          ...
          <section>
            <div className="eyebrow" style={{ marginBottom: 10 }}>Universe / allocation</div>
            <div style={{ color: "var(--text-3)", fontSize: 13 }}>Run a backtest to calculate the actual ending allocation.</div>
          </section>
        )}
      </DetailBody>
